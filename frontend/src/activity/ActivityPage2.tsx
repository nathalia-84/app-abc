...
const ActivityPage2: React.FC = () => {
  ...
  useEffect(() => {
    // Função para falar a palavra "gato"
    const speakWord = () => {
      const utterance = new SpeechSynthesisUtterance('gato');
      speechSynthesis.speak(utterance);
    };

    // Chame a função de falar a palavra "gato" quando o componente for montado
    speakWord();
  }, []);
    ...
    <div className="min-h-screen relative">
      ...
      <div className="flex items-center fixed top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div className="w-72 h-72 bg-indigo-700 rounded-lg flex items-center justify-center">
          <img className="w-64" src="src/activity/cat.png" alt="background" />
